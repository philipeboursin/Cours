\chapter{Préliminaires sur les anneaux de polynômes, idéaux, noethérianité}
    ...
    \section{Division multivariée}
        \subsection{Ordres monomiaux}
            ...
            \textbf{Contre-exemple de la démo du prof}
            Considérons la relation d'ordre $\leq_{inv}$ sur $\mathbb{N}^2$ définie comme
            \begin{align*}
                \alpha \leq_{revlex} \beta \iff \alpha \geq_{lex} \beta 
            \end{align*}
            Pour ne pas alourdir les notations, nous notons $\leq_{revlex}$ comme $\geq_{lex}$, et ainsi nous cherchons a montrer que tout partie contient un élément maximal pour $\leq_{lex}$. 
